(function ($, w, undefined) {
	...
	function Paginate() {
		...
		p.createPages = function (ft, tbody) {
			...
			var lastPage = [];
			info.pages = [];
			var rows = tbody.children('tr:not(.footable-filtered,.footable-row-detail)');
		...
		p.createNavigation = function (ft, tbody) {
			...
			$nav.off('click', 'a[data-page]').on('click', 'a[data-page]', function (e) {
				...
				if (page === 'first') {
					...
					newPage = -1;
					var first = $nav.find('.footable-page:first').find('a').data('page');
					p.createLimited($nav, info, first - info.limitNavigation);
					p.setPagingClasses($nav, info.currentPage, info.pages.length);
				} else if (page === 'limit-next') {
					newPage = -1;
					var last = $nav.find('.footable-page:last').find('a').data('page');
		...
		p.createLimited = function(nav, info, start){
			...
			var i, page,
				$prev = nav.find('li.footable-page-arrow').children('a[data-page="limit-prev"]').parent(),
				$next = nav.find('li.footable-page-arrow').children('a[data-page="limit-next"]').parent();
		...
		p.setPagingClasses = function (nav, currentPage, pageCount) {
			...
			if (currentPage >= pageCount - 1) {
				nav.find('li.footable-page-arrow').children('a[data-page="next"]').parent().addClass('disabled');
				nav.find('li.footable-page-arrow').children('a[data-page="last"]').parent().addClass('disabled');
			}
			if (currentPage < 1) {
				nav.find('li.footable-page-arrow').children('a[data-page="first"]').parent().addClass('disabled');
				nav.find('li.footable-page-arrow').children('a[data-page="prev"]').parent().addClass('disabled');
			}
		};
